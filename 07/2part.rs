...
fn interpret(amp: &mut Amp) -> Status {
    loop {
        let mut i = amp.ip;
        let mut vec = &mut amp.tape;
        let opcode = vec[i];
        ...
        match op {
            ...
            3 => {
                if amp.input.is_empty() {
                    return Status::WaitIn;
                }
                i = op_3(&mut vec, i, &mut amp.input);
            }
            4 => {
                i = op_4(&mut vec, i, c, &mut amp.output);
                amp.ip = i;
                return Status::NewOut;
            }
            5 => {
...
fn main() {
    ...
    for perm in permutations {
        ...
        for phase in perm.iter() {
            procs.push(Amp {
                tape: vec.to_vec(),
                input: vec![(*phase + 5) as i32],
                output: vec![],
                ip: 0,
            });
        }
        let mut next_in = 0;
        for (i, _) in perm.iter().enumerate().cycle() {
            procs[i].input.push(next_in as i32);
            let status = interpret(&mut procs[i]);
            if status == Status::Halt {
                break;
            }
            next_in = procs[i].output.pop().expect("No output");
        }
        max = std::cmp::max(next_in, max);
    }
    println!("max is {}", max);
}
